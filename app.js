...
function renderFilters() {
  ...
  ['leader-env','leader-round','leader-market','leader-model','strategy-env','strategy-status','strategy-market','strategy-model','upcoming-env','upcoming-round','history-env','history-status','history-season','history-team','history-model','source-status','analytics-env'].forEach(id => { const el=document.getElementById(id); if(el && !el.dataset.bound) { el.addEventListener('change', renderAll); el.dataset.bound='1'; } });
  ['leader-search','strategy-search','history-search','upcoming-search'].forEach(id => { const el=document.getElementById(id); if(el && !el.dataset.bound) { el.addEventListener('input', renderAll); el.dataset.bound='1'; } });
}
...
function matches(row, envId='leader-env', roundId='leader-round', marketId='leader-market', searchId='leader-search', modelId='leader-model') {
  const env=selected(envId), round=selected(roundId), market=selected(marketId), model=modelId?selected(modelId):'ALL', search=(document.getElementById(searchId)?.value || '').toLowerCase();
...
function renderPostseason() {
  ...
  // Upcoming postseason wagers with full reasoning inputs (spec §17).
  const poUp=arr('upcoming_bets').filter(x=>x.round_code);
  const names=Object.fromEntries(arr('players').map(p=>[p.player_id,p.name]));
  if(!poUp.length){
    $('#postseason-upcoming').innerHTML='<div class="empty">No upcoming postseason predictions. The 2026 bracket rows use placeholder team names until clinched — no wager is asserted for an undetermined matchup.</div>';
  } else {
    $('#postseason-upcoming').innerHTML=`<div class="table-wrap"><table><thead><tr><th>Decision</th><th>Round / game</th><th>Strategy</th><th>Series state</th><th>Probable pitchers</th><th>Model P</th><th>Fair</th><th>Required</th><th>Price</th><th>Edge</th></tr></thead><tbody>${poUp.map(r=>{
...
function renderStrategies() {
  const env=selected('strategy-env'), status=selected('strategy-status'), market=selected('strategy-market'), model=selected('strategy-model'), q=(document.getElementById('strategy-search')?.value||'').toLowerCase();
...
function renderUpcoming() {
  const q=(document.getElementById('upcoming-search')?.value||'').toLowerCase();
  const rows=arr('upcoming_bets').filter(r=>(selected('upcoming-env')==='ALL'||(r.env||r.environment)===selected('upcoming-env'))&&(selected('upcoming-round')==='ALL'||r.round_code===selected('upcoming-round'))&&(!q||JSON.stringify(r).toLowerCase().includes(q)));
...
function renderAnalytics() {
  ...
  const lb=arr('leaderboard').filter(x=>envSel==='ALL'||x.env===envSel);
  ...
  const cards=[['Verified price wagers', evaluated.reduce((n,x)=>n+(x.verified_bets||0),0)],['Evaluation picks', evaluated.reduce((n,x)=>n+(x.eval_picks||0),0)],['Strategies with data', evaluated.length],['Calibrated strategies', calibr.length],['Mean Brier (calibrated)', avgBrier==null?null:Number(avgBrier).toFixed(4)],['Open positions', summary().total_open_positions||0]];
  $('#analytics-cards').innerHTML=cards.map(c=>`<div class="metric"><span>${h(c[0])}</span><strong>${c[1]==null?'No data':fmtN(c[1])}</strong><small>${evaluated.length?'from published data':'No source-backed performance is available'}</small></div>`).join('');
  ...
  const calTable=calibr.slice(0,12).map(x=>`<tr><td>${h(x.id)}</td><td><span class="pill env-${h(x.env)}">${h(x.env)}</span></td><td>${Number(x.brier).toFixed(4)}</td><td>${x.log_loss==null?'—':Number(x.log_loss).toFixed(4)}</td><td>${fmtN(x.calibration_n)}</td></tr>`).join('');
  $('#analytics-detail').innerHTML=`<p class="muted">${h(clvNote)}</p>`+(calTable?`<div class="table-wrap"><table><thead><tr><th>Strategy</th><th>Env</th><th>Brier</th><th>Log loss</th><th>n</th></tr></thead><tbody>${calTable}</tbody></table></div>`:'<p class="muted">No calibration rows published yet.</p>');
}
